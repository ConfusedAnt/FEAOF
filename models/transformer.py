...
"""
from const import CONFIG_PATH_TRANS
from new_unit import get_config
...
class RobertaRegressionHead(nn.Module):
    ...
    def __init__(self, config, herg_em):
        super(RobertaRegressionHead, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        
        self.cat_liner = nn.Linear( config.hidden_size+1280, config.hidden_size)

        self.out_proj = nn.Linear(config.hidden_size, 1)
        self.sigmoid = torch.nn.Sigmoid()
        self.herg_em = herg_em
    ...
    def forward(self, features):
        ...
        x = torch.relu(x)
        x = self.dropout(x)
        if self.herg_em is not None:
            herg_em = self.herg_em.expand(x.shape[0], -1)
            x = torch.cat((x, herg_em), dim=1)
            x = F.relu(self.cat_liner(x))
            # h = h + herg_em
            x = self.dropout(x)
        x = self.out_proj(x)
        x = self.sigmoid(x)
...
class Transformer:
    def __init__(self, herg_em, batch_size, lr: int = 0.0005, freeze_core: bool = True, save_path='.',
                 epochs: int = 100, lr_decay_ratio=0.95, *args, **kwargs):
        """
        ...
        self.epochs = epochs
        self.epoch = 0
        self.save_path = os.path.join(save_path, 'Transformer_best_model.pkl')
        self.name = 'Transformer'

        # Load the model and init the regression head
        self.model = AutoModel.from_pretrained(r'/home/zhaobowen/.cache/huggingface/transformers', local_files_only=True)
        if freeze_core:
        ...
        self.model._modules['pooler'] = RobertaRegressionHead(self.model.config, self.herg_em)
        ...
        self.train_losses, self.val_losses, self.f1s = [], [], []
    ...
    def train(self, x_train: Dict[Tensor, Tensor], y_train: List[float], x_val: Dict[Tensor, Tensor] = None,
              y_val: List[float] = None, early_stopping_patience: int = None, epochs: int = 300,
              print_every_n: int = 10, *args, **kwargs):
        """
        ...
        for epoch in range(epochs):
            ...
            # As long as the model is still improving, continue training
                ...
                if self.epoch % print_every_n == 0:
                    print(f"Epoch {self.epoch} | Train Loss {loss} | Val Loss {val_loss} | patience {patience} | cur_lr {round(cur_lr,6)}")
