'''https://github.com/molML/MoleculeACE/blob/main/MoleculeACE/benchmark/featurization.py'''
"""
...
from const import RANDOM_SEED, Descriptors
from new_unit import get_config, smi_tokenizer
...
class Featurizer:
    ...
    def maccs(smiles: List[str]):
        ...
        return rdkit_numpy_convert(fp)
    @staticmethod
    def fp_des(smiles: List[str]) -> np.ndarray:

        molecular_features = merge_fp_des(smiles)
    ...
    def tokens(smiles: List[str], max_smiles_length: int = 200, padding: bool = True, truncation: bool = True,
            ...
            auto_tokenizer: str = r'/home/zhaobowen/.cache/huggingface/transformers'
               ):
        ...
        chemical_tokenizer = AutoTokenizer.from_pretrained(auto_tokenizer, local_files_only=True)
    ...
    def __call__(self, descriptor: Descriptors, **kwargs):
        ...
        if descriptor.name == 'FP_DES':
            return self.fp_des( **kwargs)        
...
class GenFeatures(object):
    ...
    def __call__(self, data):
        ...
        for atom in mol.GetAtoms():
            ...
            ### 这里如果有nan，GRAPH模型会报错：RuntimeError: CUDA error: device-side assert triggered
            if math.isnan(partial_charge):
                raise ValueError(f"{data.smiles} Partial charge is NaN")
        ...
        if len(edge_attrs) == 0:
            print("Len of this smiles'edge_attrs is 0 : ", data.smiles)
...
def featurize_graph(smiles: str):
    ...
    # fps = torch.tensor(compute_fps(smiles),dtype=torch.float32)
    # phys_chem = torch.tensor(cal_phys_chem(mol), dtype=torch.float32)
    # phys_chem = torch.tensor(descriptor[smiles].reshape(1, -1),dtype=torch.float32)
...
def compute_fps(smiles: str) -> np.ndarray:

    molecular_mols = Chem.MolFromSmiles(smiles)
    # Initialize an array to store ECFP2 & PubChem fingerprint features
    features = np.zeros((1, 1024 + 881), dtype=np.float32) ### torche默认是float32

    ECFP2_mol_fingerprint = CalculateECFP2Fingerprint(molecular_mols)
    pubchem_mol_fingerprint = CalculatePubChemFingerprint(molecular_mols)
    numerical_representation = np.concatenate(
            (ECFP2_mol_fingerprint[0], pubchem_mol_fingerprint)
        )
    features[0]=numerical_representation
    return features
...
def merge_fp_des(smiles: List[str]) -> np.ndarray:
    ...
    descriptors = compute_descriptor_features(smiles)
...
def cal_phys_chem(m):
    ...
    phys_chem = np.array([weight, logp, h_bond_donor, h_bond_acceptors, rotatable_bonds, atoms, heavy_atoms,
                        molar_refractivity, topological_polar_surface_area, formal_charge, rings],dtype=np.float32)

    return phys_chem.reshape(1,-1)
...
def save_descriptors(smiles_list, filename, file_format='pkl'):
    """
    Compute descriptors for a list of SMILES strings and save to a file.
    ...
    filename: str
        The filename to save the descriptors.
    file_format: str
        The file format to save the descriptors ('pkl' or 'json').

    Returns
    -------
    None
    """
    descriptors_2D = compute_descriptor_features(smiles_list)
    descriptors_dict = {smi: descriptors_2D.loc[i].values for i, smi in enumerate(smiles_list)}
    ...
    if file_format == 'pkl':
        ...
        with open(filename, 'w') as f:
            json.dump({smi: descriptor.tolist() for smi, descriptor in descriptors_dict.items()}, f)
    else:
        raise ValueError("Unsupported file format. Use 'pkl' or 'json'.")

def load_descriptors(filename="./data/processed/descriptors.pkl", file_format='pkl'):
    """
    ...
    ----------
    filename: str
        The filename to load the descriptors.
    file_format: str
        The file format of the descriptors ('pkl' or 'json').

    Returns
    -------
    dict
        Dictionary with SMILES as keys and descriptor arrays as values.
    """
    if file_format == 'pkl':
        ...
        with open(filename, 'r') as f:
            ...
            descriptors_dict = {smi: np.array(desc, dtype=np.float32) for smi, desc in descriptors_dict.items()}
    else:
        raise ValueError("Unsupported file format. Use 'pkl' or 'json'.")
